use crate::models::{
    ...
    MailboxRole, NormalizedMessage, NormalizedThread, ProviderKind, Suggestion, SuggestionAction,
    ThreadDetail,
};
use chrono::Utc;
...
impl Store {
    ...
    pub fn get_account_settings(&self, account_id: &str) -> Result<Value, String> {
        let connection = self.connect()?;
        let raw = connection
            .query_row(
    ...
    pub fn latest_draft_for_thread(
        &self,
        thread_id: &str,
    ) -> Result<Option<DraftDocument>, String> {
        let connection = self.connect()?;
        connection
            .query_row(
                r#"
    ...
    pub fn append_outbound_message(&self, draft: &DraftDocument) -> Result<(), String> {
        ...
        transaction
            .execute(
                ...
                params![
                    draft.thread_id,
                    draft.text.lines().next().unwrap_or("Sent reply"),
                    draft.updated_at
                ],
            )
            .map_err(|err| err.to_string())?;
            ...
            let suggestions = suggestion_statement
                ...
                .map(|row| {
                    ...
                    Ok::<Suggestion, String>(Suggestion {
                        ...
                        confidence: row.7,
                        selected: row.8 != 0,
                        draft_reply: row.9.as_ref().map(|raw| from_json(raw)).transpose()?,
                    })
                })
                .collect::<Result<Vec<_>, _>>()?;
        ...
        match action {
            SuggestionAction::Archive => {
                let archive_mailbox_id = self
                    ...
                    .ok_or_else(|| {
                    "Archive mailbox is not configured for this account.".to_string()
                })?;
                connection
                    .execute(
            ...
            SuggestionAction::MoveToInbox => {
                let inbox_mailbox_id = self
                    ...
                    .ok_or_else(|| {
                        "Inbox mailbox is not configured for this account.".to_string()
                    })?;
                connection
                    .execute(
