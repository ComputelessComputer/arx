...
use models::{
    ActionRequest, AppSnapshot, ConnectAccountInput, DraftDocument, DraftRewriteTone, MailAccount,
    ThreadDetail,
};
...
fn connect_account(app: AppHandle, input: ConnectAccountInput) -> Result<MailAccount, String> {
    ...
    let delta = adapter.sync_delta(&connection.account, None)?;
...
fn sync_account(app: AppHandle, account_id: String) -> Result<(), String> {
    ...
    let delta = adapter.sync_delta(&account, cursor.as_deref())?;
    store.save_sync_delta(
        &account_id,
        &delta.mailboxes,
        &delta.threads,
