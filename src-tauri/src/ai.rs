...
pub fn build_reply_draft(
    ...
    let latest_inbound = detail
        ...
        .unwrap_or_else(|| detail.messages.last().expect("thread should have at least one message"));
...
pub fn build_suggestion_batch(account: Option<&MailAccount>, threads: &[ThreadDetail]) -> AISuggestionBatch {
    ...
    for detail in threads {
            ...
            || detail.thread.needs_reply
        {
            let suggested_reply = build_reply_draft(
                ...
                detail,
                Some("I can take care of this and will send the updated material by Friday morning."),
            );
            ...
            Some((
                ...
                "The message looks informational and does not appear to need a response.".to_string(),
                0.63,
                None,
            ))
        } else {
...
mod tests {
    ...
    fn professional_rewrite_adds_formal_phrasing() {
        let rewritten = rewrite_draft_text("Hi,\n\nThanks for sending this.", DraftRewriteTone::Professional);
